...
contract FacetPortV670 is Upgradeable, FacetOwnable, Pausable, Initializable, FacetEIP712 {
    ...
    event OfferAccepted(bool success, string offerType, address indexed offerer, address indexed buyer, address indexed seller, address recipient, address assetContract, uint256 assetId, uint256 considerationAmount, address considerationToken, bytes16 offerId);
    ...
    function acceptOfferWithSignature(
        ...
        bool success = _acceptOfferWithSignature(offerType, offerId, offerer, recipient, assetContract, assetId, assetType, assetAmount, considerationToken, considerationAmount, startTime, endTime, signature);
    ...
    function acceptMultipleOffersWithSignatures(
        ...
        require(offerIds.length == recipients.length, "Array lengths mismatch");
        require(offerIds.length <= 20, "Cannot accept more than 20 offers at a time");

        bool atLeastOneSuccess = false;
        for (uint256 i = 0; i < offerIds.length; i++) {
            bool success = _acceptOfferWithSignature(offerTypes[i], offerIds[i], offerers[i], recipients[i], assetContracts[i], assetIds[i], assetTypes[i], assetAmounts[i], considerationTokens[i], considerationAmounts[i], startTimes[i], endTimes[i], signatures[i]);
            if (success) {
    ...
    function _acceptOfferWithSignature(
        ...
        bool transferSucceeded = _payRoyaltiesAndTransfer(assetContract, assetId, seller, buyer, recipient, considerationAmount, considerationToken);

        emit OfferAccepted(transferSucceeded, offerType, offerer, buyer, seller, recipient, assetContract, assetId, considerationAmount, considerationToken, offerId);
    ...
    function _payRoyaltiesAndTransfer(
        ...
        _transferNFT(assetContract, assetId, recipient == address(0) ? buyer : recipient, seller);

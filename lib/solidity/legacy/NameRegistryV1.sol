...
contract NameRegistryV1 is FacetERC721, Upgradeable, Initializable, Ownable, Pausable, EIP712 {
    ...
    function tokenURI(uint256 id) public view override returns (string memory) {
        ...
        Card storage card = token.cardDetails;
        string memory renderedCard = renderCard(id);
        string memory b64Card = string(abi.encodePacked("data:text/html;charset=utf-8;base64,", Base64.encode(bytes(renderedCard))));
        string memory json_data = string(abi.encodePacked('{"name":"', token.name.escapeJSON(), ' (#', id.toString(), ')", "description":"', card.bio.escapeJSON(), '", "animation_url":"', b64Card, '"}'));

...
RSpec.describe "Name Registry" do
  ...
  before do
    allow(client).to receive(:current_time).and_return(Time.zone.now)
  end
  ...
  it "deploys name registry" do
    ...
    registry_address = deploy_contract_with_proxy(
      implementation: 'predeploys/NameRegistryV2f5',
      from: from_address,
      args: [
        "Facet Cards",
        "Card",
        user_address,
        usd_wei_cents_in_one_eth,
        char_count_to_wei_usd_per_sec,
        "",
        weth_address
      ]
    ).contract_address
    ...
    initial_balance = make_static_call(
    ...
    final_balance = make_static_call(
      contract: weth_address,
      function_name: "balanceOf",
      function_args: alice
    )
    ...
    token_uri = make_static_call(
      contract: registry_address,
      function_name: "tokenURI",
      function_args: 1
    )
    
    resolved_address = make_static_call(
      contract: registry_address,
    ...
    chainid = ChainIdManager.current_l1_chain_id
    ...
    expect(JSON.parse(token_uri[/.*?,(.*)/, 1]).except('animation_url')).to eq({
      "description" => "JJ bio",
      "name" => "stickerfan (#5)",
    })
  end
